...
async def start_scheduler(bot: Bot) -> AsyncIOScheduler:
    ...
    try:
        ...
        logger.warning("Scheduler preflight: chat %s unreachable, starting anyway", chat_id)

    scheduler = AsyncIOScheduler(timezone=tz)

    scheduler.add_job(
        partial(heartbeat_tick, bot, chat_id),
        trigger=IntervalTrigger(minutes=30, timezone=tz),
        id="heartbeat",
        replace_existing=True,
    )
    scheduler.add_job(
        partial(morning_brief, bot, chat_id),
        trigger=CronTrigger(hour=7, minute=0, timezone=tz),
        id="morning_brief",
        replace_existing=True,
    )
    scheduler.add_job(
        partial(weekly_review, bot, chat_id),
